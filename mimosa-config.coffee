...
exports.config = {
  ...
  watch:
    ...
    compiledDir: "www"              # directory location of compiled web assets, can be
  ...
  ###
  copy:
    extensions: ["js","css","png","jpg","jpeg","gif","html","eot","svg","ttf","woff","otf","yaml","kml","ico","htc","htm","json","txt","xml","xsd", "vert", "frag"] 
  ...
  server:                      # configuration for server when server option is enabled via CLI
    ...
    views:                     # configuration for the view layer of your application
      ...
      path: 'src'            # This is the path to project views, it can be absolute or
  ...
  lint:                      # settings for js, css linting/hinting
    ...
    copied:                  # settings for copied files, files already in .css and .js files
      ...
      css: false              # fire csslint for copied css files
